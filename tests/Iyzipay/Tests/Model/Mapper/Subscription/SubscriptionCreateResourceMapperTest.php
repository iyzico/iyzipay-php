...
use Iyzipay\Model\Mapper\Subscription\SubscriptionCreateResourceMapper;
use Iyzipay\Model\Subscription\SubscriptionCreate;
use Iyzipay\Model\Subscription\SubscriptionCreateWithCustomer;
use Iyzipay\Tests\TestCase;

class SubscriptionCreateResourceMapperTest extends TestCase
{
    public function test_should_map_subscription_create_resource_mapper()
    {
        $json = $this->retrieveJsonFile("subscription_details.json");
        $details = SubscriptionCreateResourceMapper::create($json)->jsonDecode()->mapSubscriptionCreate(new SubscriptionCreate());
    ...
    public function test_should_map_subscription_create_with_customer_resource_mapper()
    {
        $json = $this->retrieveJsonFile("subscription_details.json");
        $details = SubscriptionCreateResourceMapper::create($json)->jsonDecode()->mapSubscriptionCreate(new SubscriptionCreateWithCustomer());
